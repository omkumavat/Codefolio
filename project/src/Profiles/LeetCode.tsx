...
import { Code, PlusCircle, X, ExternalLink, Award, Target, Brain, DeleteIcon, CheckCircle2, RefreshCwIcon, Timer, Trophy } from 'lucide-react';
...
const LeetCode = () => {
  ...
  const [hasAccount, setHasAccount] = useState(false); // Dummy state for demo
      ...
      if (currentUser) {
        if (currentUser?.username === username) {
          if (currentUser?.LeetCode) {
            setShowRefresh(true);
            setShowDelete(true);
            response = await axios.get(`http://localhost:4000/server/leetcode/fetch/${username}`);
          } else {
            setHasAccount(false);
            return;
          }
      ...
      <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        ...
        >
          <div className="absolute inset-0">
            <img
              ...
              className={`w-full h-full object-cover ${isDarkMode ? 'opacity-10' : 'opacity-2'}`}
            />
          </div>
        ...
        {hasAccount ? (
          <>
            ...
            <section className={`-mt-10 py-12 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    { icon: <Target />, label: "Total Solved", value: profile.totalSolved },
                    { icon: <Trophy />, label: "Contest Rating", value: Math.floor(contestRating) },
                    { icon: <Award />, label: "Global Rank", value: `#${profile.ranking}` },
                    { icon: <CheckCircle2 />, label: "Acceptance Rate", value: profile.acceptanceRate }
                  ].map((stat, index) => (
                    <motion.div
                      key={index}
                      initial={{ y: 20, opacity: 0 }}
                      ...
                      className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'
                        } shadow-lg`}
                    >
            ...
            <section className={`py-12 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
              <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    { type: "Easy", count: profile.easySolved, total: 856, color: "bg-green-500" },
                    { type: "Medium", count: profile.mediumSolved, total: 1793, color: "bg-yellow-500" },
                    { type: "Hard", count: profile.hardSolved, total: 796, color: "bg-red-500" }
                  ].map((level, index) => (
                    <motion.div
                      key={index}
                    ...
                    >
                      <div className="flex justify-between mb-2">
                        <span className={isDarkMode ? 'text-white' : 'text-gray-900'}>{level.type}</span>
                      ...
                      <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(level.count / level.total) * 100}%` }}
                        />
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {recentProblem.map((problem, index) => (
                        ...
                        >
                          <td className={`py-4 px-6 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                            {problem.title}
                          </td>
                          <td>
                          ...
                          <td className={`py-4 px-6 ${problem.statusDisplay === 'Accepted' ? 'text-green-500' : 'text-yellow-500'
                            }`}>
                            {problem.statusDisplay}
                          </td>
                        </motion.tr>
