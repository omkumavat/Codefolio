...
import { Code2, Plus, X, Trophy, Star, Award, Zap, School, Book, Timer, Code, Languages, Building, Target, PlusCircle, Brain, UserMinus, RefreshCwIcon, DeleteIcon } from 'lucide-react';
            ...
            if (currentUser) {
                if (currentUser?.username === username) {
                    if (currentUser?.GeeksforGeeks) {
                        ...
                        response = await axios.get(`http://localhost:4000/server/gfg/fetch-gfg/${username}`);
                    } else {
                        setHasAccount(false);
                        return;
                    }
            ...
            if (currentUser) {
                if (currentUser?.username === username) {
                    if (currentUser?.GeeksforGeeks) {
                        ...
                        response = await axios.get(`http://localhost:4000/server/gfg/fetch-gfg-db/${gfgid}`);
                    } else {
                    ...
                    <>
                        ...
                        <div className="container mx-auto px-4 py-12">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mb-12">
                                {[
                                    { icon: <Trophy />, label: 'Contest Rating', value: `${contestRating[2]}` },
                                    { icon: <Trophy />, label: 'Coding Score', value: `${contestRating[0]}` },
                                    { icon: <Star />, label: 'Problem Solved', value: `${contestRating[1]}`, subtext: 'Rising Star' },
                                    { icon: <Award />, label: 'Institute Rank', value: `${rank}`, subtext: education },
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 -mt-12">
                                            {difficultyLevels.map((item, index) => (
                                                <div key={index} className="flex items-center space-x-2">
                                                    ...
                                                    <span className="text-sm">{item.difficulty}: {item.solved}</span>
                                                </div>
                                            ))}
                                        </div>
