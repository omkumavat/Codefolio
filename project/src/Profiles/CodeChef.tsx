            ...
            if (currentUser) {
                if (currentUser?.username === username) {
                    if (currentUser?.CodeChef) {
                        ...
                        response = await axios.get(`http://localhost:4000/server/codechef/fetch-codechef/${username}`);
                    } else {
                        //account not added
            ...
            if (currentUser) {
                if (currentUser?.username === username) {
                    if (currentUser?.CodeChef) {
                        ...
                        response = await axios.get(`http://localhost:4000/server/codechef/fetch-codechef-from-db/${codechefid}`);
                    } else {
                        console.log(hasAccount)
                        setHasAccount(false);
                        return;
                    ...
                    <>
                        ...
                        <div className="container mx-auto px-4 py-12">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
                                {[
                                    { icon: <Trophy />, label: 'Current Rating', value: contestRating, subtext: `${stars[0]}★` },
                                    { icon: <Award />, label: 'Highest Rating', value: highestcontestRating, subtext: 'Peak Performance' },
                                    { icon: <Globe />, label: 'Global Rank', value: `#${globalRank}`, subtext: 'Top 5%' },
                                    { icon: <Flag />, label: 'Country Rank', value: `#${countryRank}`, subtext: countryName },
                                ].map((stat, index) => (
                                    <motion.div
                                        key={index}
                                        initial={{ y: 20, opacity: 0 }}
