...
const CombinedActivityCalender: React.FC<ActivityCalendarProps> = ({ data, isDarkMode, selectedYear }) => {
  const getIntensityColor = (submissions: number, contributions: number) => {
    const total = submissions + contributions;
    if (total === 0) return isDarkMode ? "bg-gray-800 border-gray-700" : "bg-gray-200 border-gray-300";
            ...
            <div className="grid grid-cols-7 gap-1 mt-2">
              {groupedData[monthIndex].map((day) => (
                <motion.div
                  ...
                  className={`w-4 h-4 rounded-sm border cursor-pointer relative group ${getIntensityColor(day.submissions, day.contributions)}`}
                >
                  {/* Tooltip */}
                  <div
  ...
  <ListChecks className="h-3 w-3" /> {day.submissions} submissions
</p>

                    <p className="flex items-center gap-1"><GitFork className="h-3 w-3"/> {day.contributions} contributions</p>
                    <p>{day.date}</p>
                  </div>
                </motion.div>
              ))}
