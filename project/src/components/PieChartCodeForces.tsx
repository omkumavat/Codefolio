...
interface ProblemsPieChartProps {
  problemsSolvedByRating: [number, string[]][];
  isDarkTheme: boolean;
}

export function PieChartCodeForces({ problemsSolvedByRating, isDarkTheme }: ProblemsPieChartProps) {
  ...
  const options = {
    plugins: {
      ...
      tooltip: {
        ...
        backgroundColor: isDarkTheme ? 'rgba(17, 24, 39, 0.8)' : 'rgba(255, 255, 255, 0.8)',
  ...
  return (
    <div className={`${isDarkTheme ? 'bg-gray-800' : 'bg-white'} p-6 rounded-xl shadow-sm`}>
      <h3 className={`text-lg font-semibold mb-4 ${isDarkTheme ? 'text-gray-100' : 'text-gray-800'}`}>
        Problems Solved by Rating
      </h3>
      <div className="h-[400px] relative">
        <Pie data={data} options={options} />
      </div>
      <div className={`mt-4 grid grid-cols-2 gap-4 text-sm ${isDarkTheme ? 'text-gray-300' : 'text-gray-600'}`}>
        <div>
          <span className="font-medium">Total Problems: </span>
          {problemsSolvedByRating.reduce((acc, [, problems]) => acc + problems.length, 0)}
        </div>
        <div>
