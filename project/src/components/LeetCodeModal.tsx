...
const LeetCodeModal = ({ isModalOpen,setToast ,setIsModalOpen }) => {
    ...
    const handleRefresh = async () => {
        ...
        const response = await axios.get(`http://localhost:4000/server/leetcode/check-username/${username}`);
        if (response.data.success) {
            if (response.data.total1 === (total1 + 1) && response.data.total2 === (total2 + 1)) {
        ...
        <AnimatePresence>
            ...
            {isModalOpen && (
                <>
                    ...
                    >
                        <div
                            className={`relative w-full max-w-md p-6 rounded-2xl shadow-xl ${isDarkMode ? "bg-gray-800" : "bg-white"
                                }`}
                        >
                            <button
                                onClick={() => setIsModalOpen(false)}
                                className={`absolute right-4 top-4 p-1 rounded-full ${isDarkMode ? "hover:bg-gray-700" : "hover:bg-gray-100"
                                    }`}
                            >
