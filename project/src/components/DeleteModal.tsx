...
const DeleteModal = ({ isDeleteModal, setIsDeleteModal, setToast, id, accid, acc }) => {
  ...
  const handleVerify = async () => {
    ...
    try {
      setIsVerifying(true);
      // Verify the password
      const verifyResponse = await axios.post("http://localhost:4000/server/user/verify-pass", {
        password,
        id,
      });
      ...
      if (verifyResponse.data.success) {
        ...
        const endpoints = {
          LeetCode: "http://localhost:4000/server/leetcode/delete-leetcode",
          CodeForces: "http://localhost:4000/server/codeforces/delete-codeforces",
          CodeChef: "http://localhost:4000/server/codechef/delete-codechef",
          GeeksforGeeks: "http://localhost:4000/server/gfg/delete-geeksforgeeks",
          Github: "http://localhost:4000/server/github/delete-github",
        };
        ...
        if (deleteResponse.data.success) {
          setToast({ success: true, text: `${acc} account deleted successfully` });
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-11/12 max-w-md mx-auto p-6 transform transition-all duration-300 ease-in-out">
        {/* Modal Header */}
