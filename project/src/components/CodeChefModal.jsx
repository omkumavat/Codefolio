...
const CodeChefModal = ({ isModalOpen, setToast, setIsModalOpen }) => {
  ...
  const handleVerify = async () => {
    ...
    const response = await axios.get(
      `http://localhost:4000/server/codechef/check-user/${username}`
    )
    ...
    if (response.data.success) {
      ...
      setAccount("")
      setVerificationStep1(
        `1) Visit the CodeChef for problem name "Happy New Year!".`
      )
  ...
  const handleRefresh = async () => {
    ...
    if (response.data.success) {
      if (
        response.data.problemsolved === Problem ||
        response.data.problemsolved === "NEWYEAR"
      ) {
        ...
        try {
          const response = await axios.post(
            `http://localhost:4000/server/codechef/add-codechef`,
            {
              username: username,
