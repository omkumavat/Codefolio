...
import { Search, X, Activity, ArrowRight } from 'lucide-react';
...
const CandidateSelectionModal = ({ onClose }) => {
    ...
    const fetchCandidates = async (query, setResults) => {
        if (query.length > 0) {
            try {
                const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/dashboard/comparecandidate/search/candidates?search=${encodeURIComponent(query)}`);
