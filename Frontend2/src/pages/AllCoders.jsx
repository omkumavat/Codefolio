...
const AllCoders = () => {
    ...
    return (
      ...
      >
        {error && (
          <div
            className={`p-4 rounded-lg mb-4 flex justify-between items-center animate-fadeIn ${
              isDarkMode ? "bg-yellow-700/80 text-yellow-200" : "bg-yellow-200 text-yellow-700"
            }`}
          >
            <p>{error}</p>
            <button
              onClick={fetchData}
              disabled={isLoading}
              className={`px-4 py-2 rounded-md flex items-center transition-transform ${
                isDarkMode ? "bg-yellow-600 hover:bg-yellow-500" : "bg-yellow-400 hover:bg-yellow-300"
        ...
        <div
          className={`w-full p-4 rounded-2xl border grid grid-cols-1 md:grid-cols-3 gap-4 ${
        ...
        >
          <div className="relative">
            ...
            <select
              className={`w-full h-12 rounded-lg pl-10 pr-10 border focus:border-blue-500 appearance-none ${
                isDarkMode ? "bg-gray-800 text-gray-300 border-gray-600" : "bg-white text-gray-700 border-gray-300"
              }`}
        ...
        <div
          className={`p-4 rounded-2xl border relative flex gap-4 ${
            isDarkMode ? "bg-gray-800/60 border-gray-700" : "bg-gray-100 border-gray-300"
          }`}
        >
          <div className="w-full relative">
            ...
            <input
              ...
              className={`w-full h-12 rounded-lg pl-10 pr-12 border focus:border-blue-500 ${
          ...
          <input
            ...
            className={`h-12 rounded-lg pl-3 pr-3 border focus:border-blue-500 w-32 ${
              isDarkMode ? "bg-gray-700 text-gray-200 border-gray-600" : "bg-white text-gray-700 border-gray-300"
            }`}
          />
        </div>
        ...
        <div
          className={`overflow-x-auto p-4 rounded-2xl border ${
            isDarkMode ? "border-gray-700" : "border-gray-300"
          }`}
        >
          <table ref={tableRef} className="min-w-full table-auto">
            <thead>
              <tr className="text-left">
                <th className={`p-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>No.</th>
                <th onClick={() => handleSort("username")} className={`cursor-pointer p-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                  Username <SortIcon field="username" />
                </th>
                <th onClick={() => handleSort("name")} className={`cursor-pointer p-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                  Name <SortIcon field="name" />
                </th>
                <th onClick={() => handleSort("college")} className={`cursor-pointer p-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                  College <SortIcon field="college" />
                </th>
                <th onClick={() => handleSort("branch")} className={`cursor-pointer p-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                  Branch <SortIcon field="branch" />
                </th>
                <th onClick={() => handleSort("solved")} className={`cursor-pointer p-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                  {platform === "Github" ? "Contributions" : "Solved"} <SortIcon field="solved" />
                </th>
                <th onClick={() => handleSort("totalActiveDays")} className={`cursor-pointer p-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
            ...
            <tbody>
              {(userRange.start !== 1 || userRange.end !== 1 ? rangeFilteredUsers : paginatedUsers).map(
                (user, index) => (
                  <tr key={user.username} className="hover:bg-gray-100 hover:text-black">
                    <td className="p-2">
